...
- An important nuance is that this function always calculates the entire area to the left of a specificed x value. So, if we are looking for a range between two values, we would have to subtract the smaller area to the left of the specified range value from a larger area to the left of the second range value. An example of this is shown below:
...
Similar to normal distribution funcions, the *'dbinom()'* function gives the density. It helps us find the individual probabilities as well as a range of individual probabilities. In orther words, it will provide the probability of a number of successful tirals (*x*), from a number of total trials (*size*), based on a probability of the positive result of each trial (*prob*). 
